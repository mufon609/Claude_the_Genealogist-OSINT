...
3. **Every value a held record gives for a fact feeds the searches, whether or not it is the tree's.** The search foundation carries, per field, the Accepted value with basis `accepted` and every other value a held record states for the same fact with basis `record` and the record named, the way it already carries name variants and aliases. A built search query or URL uses the Accepted value and offers the variants on the step with the same include control the name variants have, so a search that needs a birth place for Noi Davidson can be run on Iwate and on Tokushima. The plan regenerates on every accept, so a value taken from a record under item 1 changes the queries the next run builds; that is the point: new evidence makes the next search better, and the loop goes back before it goes forward.
4. Test on scratch: Noi's burial takes the cemetery and plot from the memorial; Raymond's birth takes Babylon; a second run offers nothing more; the file's claim remains visible as its own row; Noi's foundation shows both birth places with their bases and a search step built for her carries both, with the record's one marked as such.
