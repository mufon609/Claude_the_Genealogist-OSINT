...
| `tools/resolve_places.py` | Resolve `place_string` rows via Nominatim: parse + normalize, verify every given component against the candidate's hierarchy, auto-accept only unique full matches, everything else — including a place and its enclosing unit of the same name verifying together — stays Undecided with a tree-scoped `place_resolution` proposal carrying every verified candidate. Then fills `event.place_id` only where every supporting fact resolved to the same place (audit-logged per event). `--reset` undoes AI-made resolutions and keeps human ones. One audit row per string accepted, rejected or reset, under `--by`. Overrides in `data/place-overrides.json`. Responses cached under `derivatives/geocode/`. |
