...
| `tools/attach_inbox.py [file ...]` | Attach every inbox file (or the named ones) to the fetch steps its own identity fulfils: the memorial id or ark read from the file (kept as an `artifact_locator` too), the file archived once, a found run logged on each step, then the extractor and matcher once. A file matching no step stays in the inbox. `tools/attach.py` is the shared path the person screen's attach uses too. |
