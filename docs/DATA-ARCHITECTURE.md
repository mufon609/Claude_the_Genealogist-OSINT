...
3. **Living-person policy: two thresholds.** `record_release` follows each
   ...
   death evidence, manual override allowed, searched at no source on their
   name alone (their search steps are assisted; a record fetched for a
   relative that names them attaches to them as to anyone), redacted in every
   export and
