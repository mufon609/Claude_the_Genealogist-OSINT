...
| `tools/log_search.py --step <id> --outcome …` | Record a run (found / none / blocked / error) with the step's fields as rendered after include/revise; a run logged `error` is a source that did not answer, not a run on the fields, so the step stays runnable and the next turn asks the source again; `--dismiss <question id>` closes a question for good; `--list "<person>"` shows the plan with outcomes. |
