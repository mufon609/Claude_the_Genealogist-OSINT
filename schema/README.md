...
| `tools/extract.py <sha256 or path>` | Parse an archived record page (HTML) into one extraction: a Find a Grave memorial by `rule:findagrave-memorial@0.1.0` (verified on a real memorial), a Find a Grave search results page by `rule:findagrave-search@0.1.0` (one persona per row, the memorial id and URL as its identity; verified on two real pages), any other page as an Ancestry index page by `rule:ancestry-index@0.1.0`; a persona per person named, a fact per field as written, a relation per stated relationship, the raw parsed page in `structured_json`. Run on arrival by the person screen's attach; a second run supersedes the first and rejects its undecided proposals. |
| `tools/match.py <extraction id>` | Compare every persona of an extraction with the persons the record was fetched for (the step it came through, every fetch step naming the same record or census page) and their relatives on name, sex, birth year and stated relationships; write one `persona_match` or `new_person` proposal per persona with a plain-words rationale. Run on every extraction as it is written; re-running adds nothing. |
| `tools/conclude.py` (module) | What an accepted persona writes on the tree: Undecided assertions from its facts onto the person and their events and attributes, the family links the record states with persons already matched on it; and `accept_record_facts`, one decision that sets Accepted on every Undecided assertion a held record makes on a person that does not disagree with a value already Accepted. Used by the person screen and by the extractor when a re-run carries a decided link forward. |
