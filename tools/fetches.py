...
a page from any other holder takes the name the list printed with its year and record id filled in): the leads from held
records first (a persona accepted as a person, whose memorial the record links), then the file's citations, the pages
that settle most steps first. `collect` moves every saved page from the browser's download folder into `inbox/` and
attaches each: a memorial, a FamilySearch record, an AAD record or a photograph by its own identity (tools/attach.py),
archived once, logged found on every step that cites it, extracted, matched, the rule run; a page from any other holder
by the name the list printed, to the steps of the page it was saved for, archived under that holder with the page's own
URL (the saved-from line the browser wrote) as locator, logged found, and reported unparsed until a parser claims it.
"""
...
def named_for(name, entries):
    """The waiting page a saved file's name was printed for: the list's name with its year and record id filled in."""
    fill = {"<year>": r"[^-]+", "<record id>": r".+?", "<ark id>": r"[A-Za-z0-9_:-]+"}
    for e in entries:
        rx = "".join(fill.get(part, re.escape(part)) for part in re.split(r"(<year>|<record id>|<ark id>)", e["save_as"]))
...
def collect(cx, tree_id, slug, by, folder=None):
    ...
    its name); a page from any other holder, saved under the list's name with its year and record id filled in, moved to
    inbox/ and attached to the steps of the page it was saved for, archived under that holder with its own URL as locator.
