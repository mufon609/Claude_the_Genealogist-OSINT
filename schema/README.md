...
| `tools/check.py` | Green in one command: every tool compiles; every parser read against its saved real page under `tests/fixtures/` on a scratch catalog under a temporary data root, the personas, facts and relations checked against the page; `--show` prints what each wrote, `--keep` leaves the scratch in place. |
