...
| `tools/attach_inbox.py [file ...]` | Attach every inbox file (or the named ones) to the fetch steps its own identity fulfils: the memorial id or ark read from the file (kept as an `artifact_locator` too; the identity also names the holder the page came from and, for a FamilySearch record, its own collection), a gravestone photograph by the name the fetch list gave it (an image carries none in its bytes; archived under E05, not parsed), the file archived once, a found run logged on each step (a record of no census page reaches every planned step on a person of its principal's name whose citation is of the page's collection at its holder, whatever checklist row the citation sits under, and a step with no citation of its own by its row's kind: an obituary collection the obituary row, a death index the death record row), then the extractor and matcher once. A results page (told from a record page by the parser that claims it) that is the same search saved again, the same rows already logged on a step it fits, answers nothing new: a none run is logged on the step's current fields (`log_search.step_source`, the holder or the row's first source, read the same way `tools/fetches.py`'s list reads it) wherever the step wasn't already answered on those fields, the note naming the earlier artifact, and the file leaves the inbox with nothing archived a second time. A file matching no step stays in the inbox. `tools/attach.py` is the shared path the person screen's attach uses too. |
...
| `tools/fetches.py list / collect` | Every planned fetch step at a holder without a connector whose own pages the page-saves-itself method can save, once per page (a page is open for a step while the step's own source, the holder or the row's first source, has no run on its current fields; a row source's connector answering the same step does not stand for the holder's page): the holder, the link (the memorial page; the holder's search prefilled from the citation's details; a gravestone photograph's own image), the people waiting on it, and the file name to save under (a census collection's carries the row's own year, so a person's two census searches, 1925 New York state and 1930 federal, do not share a name), said to be an image where it is; leads from held records first. A step at a browse-only holder (`catalog.browse_only`: a FamilySearch images-only collection, browsed by hand, film by film) stays on the plan, fetchable, with the reason in its rationale, but never reaches this list. `collect` moves the pages and photographs saved by the browser from the download folder (or `--folder`) into `inbox/` and attaches each by its own identity — a photograph's is in its name; any other page's is read from its saved-from line (a FamilySearch record or search URL, a Find a Grave memorial or search, an AAD record or search), whatever the file is named — and a page from a holder without a parser by the name the list printed, which carries the citation's own record locator and ends in the six characters of the person it was listed for: to that person's steps on that citation, archived under the holder with its own URL, logged found, unparsed until a parser claims it. A file with neither is left in the folder. A results page saved again with rows already logged on the step is a none run on the step's current fields (the note naming the earlier artifact), the file removed with nothing archived twice. |
