...
def build(cat: Catalog, pid: str):
    ...
    if b and given and surname:
        dup = cat.q("""SELECT p.id, p.display_name FROM person p JOIN person_name n ON n.person_id=p.id AND n.is_primary
                       JOIN event_participant ep ON ep.person_id=p.id JOIN event e ON e.id=ep.event_id AND e.event_type='Birth'
                       WHERE p.tree_id=? AND p.id<>? AND lower(n.surname)=lower(?) AND lower(n.given)=lower(?) AND substr(e.date_start,1,4)=?""",
                    cat.tree_id, pid, surname, given, str(b))
        for did, dn in dup: questions.append({"kind": "duplicate_person", "detail": f"{dn} [{did}] has the same name and birth year"})
    ...
    return {"person": {"id": pid, "name": p["name"], "sex": sex, "span": [b, d], "notes": notes}, "baseline": baseline, "foundation": foundation,
            "questions": questions, "checklist": {"A": A, "B": B}}
