...
| `no-browser-sources.md` | next, second worker | Survey the registry's free sources with documented endpoints against this family, build connectors for the ones that fill the most gaps, run them on scratch and report the results as decision cards. |
